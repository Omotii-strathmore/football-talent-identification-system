...
class PasswordResetConfirmForm(forms.Form):
	code = forms.CharField(
		...
		widget=forms.TextInput(attrs={'placeholder': 'Enter the code sent to you'}),
	)
	new_password = forms.CharField(
		label='New password',
		help_text='Use at least 8 characters with uppercase, lowercase, number, and a special character. Must differ from your current password.',
		widget=forms.PasswordInput(attrs={'placeholder': 'Enter a new password'}),
	)
	confirm_password = forms.CharField(
		label='Confirm new password',
		widget=forms.PasswordInput(attrs={'placeholder': 'Repeat the new password'}),
	)
	...
	def __init__(self, *args, user=None, **kwargs):
		...
		self.helper.form_method = 'post'
		self.helper.form_tag = False
		self.helper.layout = Layout('code', 'new_password', 'confirm_password')
	...
	def clean(self):
		...
		if password:
			...
			if self.user and self.user.check_password(password):
				self.add_error('new_password', 'New password must be different from your current password.')

			if not re.search(r'[A-Z]', password):
				self.add_error('new_password', 'Password must include at least one uppercase letter.')
			if not re.search(r'[a-z]', password):
				self.add_error('new_password', 'Password must include at least one lowercase letter.')
			if not re.search(r'\d', password):
				self.add_error('new_password', 'Password must include at least one number.')
			if not re.search(r'[^A-Za-z0-9]', password):
				self.add_error('new_password', 'Password must include at least one special character.')
